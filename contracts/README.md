...
Anyway, this required some creative engineering, since Godot needs to create a hash that takes into account abi.encode.  Ultimately I solved this with Ethers-rs, which has an AbiEncode trait just for this purpose.  Combined with the OpenSSL SHA256, Godot Rust can produce the hash no problem.   Now I just need to create the UI that puts this all together in-game.
